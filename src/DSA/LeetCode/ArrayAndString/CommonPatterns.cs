...
public class CommonPatterns
{
    ...
    //
    // Many problems will ask you to return a string, and usually, this string will be built during the algorithm. Let's say the final string is of length n, and we build it one character at a time with concatenation. What would the time complexity be? The operations needed at each step would be 1 + 2 + 3 + ... + n. This is the partial sum of this series, which leads to O(n2)O(n2) operations.
    //
    //     Simple concatenation will result in an O(n2)O(n2) time complexity if you are using a language where strings are immutable.
    //
    //     There are better ways to build strings in just O(n)O(n) time. This will vary between languages - here, we'll talk about Python and Java - if you're using another language, we recommend researching the best way to build strings in your language.
    ...
    //
    //     The size of a subarray between me and j (inclusive) is j - i + 1. This is also the number of subarrays that end at j, starting from i or later.
    ...
    //
    // Typically, subsequence problems are more challenging. Because this is only the first chapter, it is challenging to talk about subsequence patterns now. Subsequences will come up again later in the course - for example, dynamic programming is used to solve a lot of subsequence problems.
    //
